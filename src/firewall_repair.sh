...
if [ "$MANAGER" = firewalld ]; then
  ...
  if $ENABLE; then root "Enabling firewalld" systemctl enable --now firewalld || true; fi
  ...
  if $ENABLE; then root "Enabling UFW" ufw --force enable || true; fi
  ...
  if $RELOAD; then root "Reloading UFW" ufw reload || true; fi
fi
